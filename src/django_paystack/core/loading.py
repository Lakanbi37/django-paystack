...
from django_paystack.core.exceptions import AppNotFoundError, ClassNotFoundError, ModuleNotFoundError
from django_paystack.utils.deprecation import RemovedInDjangoPaystack31Warning

# To preserve backwards compatibility of loading classes which moved
# from one Paystack module to another, we look into the dictionary below
...
def get_class_loader():
    return import_string(settings.DJANGO_PAYSTACK_DYNAMIC_CLASS_LOADER)
...
def default_class_loader(module_label, classnames, module_prefix):
    ...
    # import from Paystack package (should succeed in most cases)
    # e.g. 'oscar.apps.dashboard.catalogue.forms'
    paystack_module_label = "%s.%s" % (module_prefix, module_label)
    paystack_module = _import_module(paystack_module_label, classnames)
    ...
    if app_name.startswith('%s.' % module_prefix):
        # The entry is obviously an Paystack one, we don't import again
        local_module = None
    else:
    ...
    if paystack_module is local_module is None:
    ...
    return _pluck_classes([local_module, paystack_module], classnames)
...
def _find_registered_app_name(module_label):
    """
    Given a module label, finds the name of the matching Paystack app from the
    Django app registry.
    """
    ...
    if not isinstance(app_config, PaystackConfig):
        raise AppNotFoundError(
            "Couldn't find an Paystack app to import %s from" % module_label)
...
def get_profile_class():
    ...
    # makes sense for Paystack to continue to use it. Projects built on Django
...
def feature_hidden(feature_name):
    """
    Test if a certain Paystack feature is disabled.
    """
    return (feature_name is not None
            and feature_name in settings.PAYSTACK_HIDDEN_FEATURES)
...
def get_model(app_label, model_name):
    ...
    """
    paystack_moved_model = MOVED_MODELS.get(app_label, None)
    if paystack_moved_model:
        if model_name.lower() in paystack_moved_model[1]:
            original_app_label = app_label
            app_label = paystack_moved_model[0]
            warnings.warn(
                'Model %s has recently moved from %s to the application %s, '
                'please update your imports.' % (model_name, original_app_label, app_label),
                RemovedInDjangoPaystack31Warning, stacklevel=2)
    try:
...
def is_model_registered(app_label, model_name):
    ...
    register Paystack models if they aren't overridden by a forked app.
    """
    try:
